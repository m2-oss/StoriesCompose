...
internal class ExoPlayerVideoManager(private val playerPool: PlayerPool) : StoryVideoManager {
    ...
    override fun getVideoStoriesId(storiesIndex: Int, storiesId: String): UiVideo? {
        val mediaItemIndex = getCurrentPlayer(storiesIndex).currentMediaItemIndex
    ...
    override fun resumeVideo(storiesIndex: Int) {
        getCurrentPlayer(storiesIndex).play()
    }
    ...
    override fun stopVideo(storiesIndex: Int) {
        getCurrentPlayer(storiesIndex).stop()
    }

    private fun getCurrentPlayer(storiesIndex: Int): ExoPlayer =
        (playerPool.get(storiesIndex) as ExoPlayerHolder).player
